...
This is optionally returned using either of the `win_fixed` or `win_loci` arguments, which control the strategy and size for establishing regions (more information below).
...
`win_fixed` and `win_loci` are optional arguments that offer users `dar_region` values in conjunction with `dar_origin` values.
While only one of these two arguments is required, if both are provided, `win_fixed` takes precedence.
...
- `win_fixed`: Establishes a region with a fixed size around the origin.
The total region size is defined in base pairs, meaning that the region will extend `win_fixed / 2` base pairs either side of the origin.
For example if `win_fixed = 101` is specified, an origin at position 500 will have an associated region spanning positions 400-600.
- `win_loci`: Establishes an elastic region to average the specified number of loci with `dar_origin` values, such that this number of values are included.
Note that whilst the number of points around the central value may be equal, the genomic distances may not be.
In contrast to `win_fixed`, `win_loci` results in DAR estimates that cover a substantial proportion of each chromosome (i.e from the first locus to the last), which is useful for downstream assignment of DAR values to a greater number of genomic features.

The remainder of this vignette uses examples produced from specifying the `win_loci` argument.

```{r win_loci}
win_loci <- 5
```

With a chosen elastic region size of `r win_loci` loci, this will smooth the DAR metric at each origin locus with the DAR values of the `r (win_loci - 1) / 2` loci either side.

```{r dar}
dar <- dar(props = props, contrasts = contrasts, win_loci = win_loci)
```
...
The size of this region is controlled with the `win_loci` argument in the `dar()` function, which we previously set as `r win_loci`.
...
gene_dar <- readGenotypes(vcf) |>
    ...
    filterLoci() |>
    countsToProps() |>
    dar(contrasts = contrasts, win_loci = win_loci) |>
